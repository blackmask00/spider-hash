...
def crack_hash(hash, wordlist=None, hash_type='md5', min_length=0, max_length=0, characters=string.ascii_letters + string.digits, max_workers=4):
    ...
    if hash_fn is None or hash_type not in hash_name:
        raise ValueError(f'[🕸️]--> Invalid hash type: {hash_type} supported are {hash_name}')
    
    if wordlist:
        with open(wordlist, 'r') as f:
            lines = f.readlines()
            total_lines = len(lines)
            print(f"[🕸️]--> Cracking hash {hash} using {hash_type} with a list of {total_lines} passwords.")
        ...
        print(f'[🕸️]--> Cracking hash 🕷 {hash} using {hash_type} with generated passwords of lengths from {min_length} to {max_length}. Total combinations: {total_combinations}.')
...
if __name__ == '__main__':
    ...
    if cracked_password:
         print(f"[☣︎] seccussefuly Found Password 🕷: {cracked_password}")
    else:
